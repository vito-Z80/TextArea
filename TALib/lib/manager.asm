...
//-------------------------------------------
	include "print.asm"
	include "printText.asm"
	include "selectable.asm"
	include "window.asm"
moveKeys:	db "9","8","Q","A"	; up,down,up,down
actionKeys:	db #5E,#7B,"0"	; up to three action keys
		; #5E - SPACE key
		; #7B - ENTER key
data:		equ print.data
init:
...
buildWithChooser:
	call print.build
	call selectable.keyListener
	call selectable.update
	jr buildWithChooser+3
...
setColor:
	...
	; C - sides color
	; B - nooks color
	ld (print.data + windowSettings.areaColor),a
	ld (print.data + windowSettings.sidesColor),bc
	ret
waitAnyKey:
	call selectable.keyListener
	call selectable.waitAnyKey
	jr z,$-6
	ret
